...
## We start be loading some packages and setting up the variables
```{r}
library(tidyverse)
library(magrittr)
```
...
p-values are a bit skewed to the left (as our data is non-Gaussian)
...
p-values are more or less uniformly distributed. However, there are some discrepencies (mostly due to the small sample size maybe)
...
data <- read.csv('/Users/matanatmammadli/Desktop/R codes/lung_data.csv')
head(data)
view(data)
```

```{r}
fig <- plot_ly(data, x = ~Lung.function, color = ~Trial.arm, type = "box")
fig
```

```{r}
ggbar <- ggplot(data) + aes(x = Trial.arm, y = Lung.function, fill = Trial.arm) +
  geom_bar(stat = "identity")
ggbar
ggplotly(ggbar, tooltip = c("x", "y"))
```

```{r}
fig1 <- plot_ly(data, x = ~Lung.function, color = ~Trial.arm, type = "box")
fig2 <- plot_ly(data, x = ~Lung.function, y = "ID", type = "box")
...
```{r}
fig <- plot_ly()%>%
  add_trace(data = data, x = ~Trial.arm, y = ~Lung.function, color = ~Trial.arm, type = "scatter", mode = "markers")
fig
```
...
```
```{r}
##Trial_arm <- data_tbl$Trial.arm
##Trial_arm <- as.factor(Trial.arm)
data_tbl %>%
  mutate(Trial.arm = factor(data_tbl$Trial.arm, levels = c("Control", "Treatment")))
contrasts(Trial.arm)
```
```{r}
column_vec <- names(data_tbl)
```

```{r}
control <- data_tbl[which(data_tbl$Trial.arm=="Control"), ] #make a table for the results of the control group only 
treatment <- data_tbl[which(data_tbl$Trial.arm=="Treatment"), ]#make a table for the results of the treatment group only
```

```{r}
## generalised linear model, linear regression
lung_function <- data$Lung.function
glm(Trial.arm ~ lung_function, data = data_tbl, family = "gaussian")
```

### Boxplot von Lung function data
```{r}
library("ggplot2")

ggplot(data_tbl, aes(lung_function, Trial.arm)) +
  geom_boxplot(fill="lightblue") +
  geom_jitter()

ggplot(data_tbl = control, aes(lung_function)) + 
  geom_bar(stat = "count",fill="pink")
  
##ggplot(klausur_tbl, aes(CRP, POD)) +
##  geom_boxplot() +
##  geom_jitter()
##
ggplot(data_tbl, aes(lung_function)) +
  geom_histogram(aes(fill = Trial_arm), color = "blue", binwidth = 2)

mosaic::mean(POD ~ bloodpressure, data = klausur_tbl)
```
### Barplot of Lung function data
```{r}
ggplot(data_tbl, aes(lung_function, Trial.arm)) +
  geom_bar() 
```
### Bee swarm plot von lung function data
```{r}
## x = kontinuerliche variable, y = binaer/dichotom

ggplot(data_tbl, aes(x= lung_function, y= Trial.arm)) + 
  geom_point(size=2)

ggplot(control, aes(control$Lung.function,control$Trial.arm))+
  geom_bar(color = "blue")

mean(data_tbl$lung_function)

sd(data_tbl$lung_function)

control_mean <- mean(control$Lung.function) # 10.15
treatment_mean <- mean(treatment$Lung.function) # 10.475
```

### Violin Plot von Lung function data
```{r}
ggplot(data_tbl, aes(x = Trial.arm, y = Lung.function,  fill = Trial.arm)) +
  geom_violin() +
  labs(x = "Group", y = "Lung function",
       title = "Violin plot of Lung function by group")
```

## Density plot von Lung function data
```{r}
ggplot(data_tbl, aes(x = Lung.function, fill = Trial.arm)) +
...
```{r}
t.test(Lung.function ~ Trial.arm, data = data_tbl)
```


```{r}
## Binary logistic regression (one independent/continuous variable and one dependent binaer variable)

logistic_model <- glm(data_tbl$Trial.arm ~ lung_function, family = binomial(), data_tbl)

```
```{r}
##Splitting the data into Control and Treatment group
X<-split(data_tbl, data_tbl$Trial.arm)
X
```

```{r}
# Predicting in the dataset
pred_prob <- predict(logistic_model, X, type = "response")
pred_prob
```
```{r}
##Null Model (nur Intercept) 

null_model <- glm(Trial.arm ~ 1, data = data_tbl, family = gaussian)
